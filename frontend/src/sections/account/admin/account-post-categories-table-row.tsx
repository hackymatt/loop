...
type Props = {
  row: IPostCategoryProps;
  onEdit: (category: IPostCategoryProps) => void;
  onDelete: (category: IPostCategoryProps) => void;
};

export default function AccountCategoriesTableRow({ row, onEdit, onDelete }: Props) {
    ...
    <>
      ...
      >
        <MenuItem onClick={handleEdit} sx={{ mr: 1, width: "100%" }}>
          ...
          <Typography variant="body2">Edytuj kategorię</Typography>
        </MenuItem>
        ...
        <MenuItem onClick={handleDelete} sx={{ mr: 1, color: "error.main", width: "fit-content" }}>
          ...
          <Typography variant="body2">Usuń kategorię</Typography>
        </MenuItem>
      </Popover>
    </>
  );
