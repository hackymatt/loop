...
import { ICourseLessonProp, ICourseByCategoryProps, ITeachingProp } from "src/types/course";
...
export default function LessonEditForm({ teaching, onClose, ...other }: Props) {
  const { data: availableTechnologies } = useTechnologies({
    sort_by: "name",
  });

  const { data: lessonData } = useLesson(teaching.id);
  const { mutateAsync: editLesson } = useEditLesson(teaching.id);
