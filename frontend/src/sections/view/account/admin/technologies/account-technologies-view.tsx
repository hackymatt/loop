...
export default function AccountTechnologiesView() {
  ...
  const { data: technologies, count: recordsCount } = useTechnologies(filters);
  ...
  const [editedTechnology, setEditedTechnology] = useState<ITechnologyProps>();
  const [deletedTechnology, setDeletedTechnology] = useState<ITechnologyProps>();
    ...
    <>
      <Stack direction="row" spacing={1} display="flex" justifyContent="space-between">
        ...
        <Stack direction="row" spacing={1}>
          <DownloadCSVButton queryHook={useTechnologies} disabled={(recordsCount ?? 0) === 0} />
          <LoadingButton
            component="label"
            variant="contained"
            size="small"
