...
import { useCandidates, useCandidatesPagesCount } from "src/api/candidates/candidates";
...
import CandidateDeleteForm from "./candidate-delete-form";
...
const TABLE_HEAD = [
  { id: "name", label: "Nazwa kandydata", minWidth: 200 },
...
export default function AccountCoursesCandidatesView() {
  const newCandidateFormOpen = useBoolean();
  const editCandidateFormOpen = useBoolean();
  const deleteCandidateFormOpen = useBoolean();

  const { setQueryParam, removeQueryParam, getQueryParams } = useQueryParams();

  const filters = useMemo(() => getQueryParams(), [getQueryParams]);

  const { data: pagesCount } = useCandidatesPagesCount(filters);
  const { data: candidates, count: recordsCount } = useCandidates(filters);
    ...
    <>
      <Stack direction="row" spacing={1} display="flex" justifyContent="space-between">
        ...
        <Stack direction="row" spacing={1}>
          <DownloadCSVButton queryHook={useCandidates} disabled={(recordsCount ?? 0) === 0} />
          <LoadingButton
            component="label"
            variant="contained"
            size="small"
            color="success"
            loading={false}
            onClick={newCandidateFormOpen.onToggle}
          >
      ...
      <CandidateNewForm open={newCandidateFormOpen.value} onClose={newCandidateFormOpen.onFalse} />
      {editedCandidate && (
        <CandidateEditForm
